...
// Flat, and it has been graded once and put back. D09 replaced this with 5% for a body picked
// up on a full clock, rising 9 points for each of its own turns spent on the floor - the same
// total scarring against the clocks measured, moved onto the falls that cost something. It
// reads well and it costs the game two sectors of depth. Seven 14-expedition samples, grouped
// by the one thing a bisect isolates:
//
//   ladder present   deepest sector 3, 3, 3        nodes 50, 59, 60
//   ladder absent    deepest sector 5, 5, 6, 7     nodes 80, 89, 90, 100
//
// The fourth "absent" arm is the bisect: this file with the ladder flattened and everything
// else the phase added still in place. It sits with the untouched builds, which is what
// convicts the ladder rather than the rest of it.
//
// The count is not what moved - scars dealt per run overlap completely across every arm, 0.93
// to 1.64. What moves is WHERE they land. An operator lies on the floor longest in the fights
// that are hardest, which are the deep ones, so a ladder charged against time spent down
// quietly relocates the same scarring out of the sectors that absorb it and into the ones that
// end a run. A mean that holds while the placement shifts was the wrong statistic twice in one
// phase - the health share went the same way for the same reason.
//
// Perfect separation across seven samples is p = 0.03 on a permutation test, which is
// suggestive rather than settled. It does not need to be settled: the phase this bought was
// built on a premise that turned out to be a miscount, so there is nothing here worth carrying
...
// ── M04: the five stat cards, made situational - and then re-cut on the measurement ─────
// M02's census said these five are 91% of every perk point the game ever spends: they are not
// filler beside the signatures, they ARE long-run progression. And they were five flat bumps
// that landed identically on every body, so the promotion screen was an ordering rather than a
// decision, and the ordering never changed from one operator to the next.
//
// THE FIRST CUT OF THIS COST SIXTEEN WINS OF A CAREER, and the reason is written into the shape
// of what is here now. A probe counted how often each condition actually fired, over 14,559
// player swings, and three of the five almost never did. The two findings that matter:
//
//   1. A CONDITION HAS TO BE A PROPERTY OF THE BODY. HONED's first condition was "the target is
//      further off than arm's reach", and `dist` is the target's index in the living-enemy
//      list - a fact about whichever foe the targeting picked, not about the operator holding
//      the card. 81% of every swing in the game lands on the front of the enemy line, so the
//      only multiplicative offence axis the player has went from always-on to one swing in
//      five. The verb's OWN reach is a body property - a Medic carries a pistol, a Bruiser
//      carries a blade - and fires on 60%. That is the axis it should have been on.
//
//   2. MAX HEALTH HAS TO STAY MAX HEALTH. The first cut turned +25 HP and +10% max HP into a
//      flat cut off each blow, on the reasoning that health cannot be conditional without the
//      bar jumping mid-fight. Restoring only those two recovered ten of the sixteen wins, and
//      the multiplicative repair that looked obvious - a share off each blow instead of a flat
//      number - measured no better than the thing it was meant to fix. Health is not just a
//      per-hit quantity here: it carries BETWEEN fights, so it is buffer a percentage cut never
//      gives back. The way to make it conditional is to gate it on a state that only changes at
//      the Outpost, which is where stats are allowed to move.
//
// So the axes are the two that are genuinely about the operator, each an opposed pair, plus one
// card that asks nothing:
//
//   VETERAN / HONED       the verb it carries:  a blade, or something with reach.
//   HARDENED / SWIFT      where it is deployed: the front rank, or behind it.
//   FORTIFIED             nothing. The card for a body you have not committed to a job.
//
// Keeping one unconditional card is deliberate rather than a leftover. Four conditional cards
// and no plain one would mean a screen that can only offer a body a mismatch, and a perk is
// permanent - there has to be something to take when the answer is "not yet".
const PERK_DMG_FLAT = 6;     // VETERAN,   per stack, added to the swing on a melee verb
const PERK_DMG_MULT = 1.12;  // HONED,     per stack, compounding, on a ranged verb
const PERK_MAXHP    = 0.12;  // HARDENED,  per stack, compounding, in the front rank
const PERK_SPD      = 4;     // SWIFT,     per stack, deployed off the front rank
const PERK_HP_FLAT  = 25;    // FORTIFIED, per stack, wherever it stands
...
// VETERAN: the hand that swings. Flat, and it goes into baseDmg rather than the multiplier
// chain so it reaches the splash and follow-up hits exactly as `dmgBase += 5` used to - several
// of those are figured off baseDmg with no multiplier at all.
function perkDmgFlat(ent, reach) {
    if (!ent || !ent.isPlayer || reach !== 'melee') return 0;
    return perkStacks(ent, 'VETERAN') * PERK_DMG_FLAT;
}
// HONED: the edge on something with reach. effReach, not the move's row in MOVE_REACH - F09's
// finding was that those two disagree for a bayoneted Pipe Rifle, and the resolver already
// works out which one this operator is actually holding.
function perkDmgMult(actEnt, reach) {
    const n = perkStacks(actEnt, 'HONED');
    return (n && reach === 'ranged') ? Math.pow(PERK_DMG_MULT, n) : 1;
}
// HARDENED and SWIFT are stats on the sheet, not live reads, because max health cannot be a
// live read and the measurement says that currency is the one that carries. They are
// RECONCILED rather than incremented: this works out what the body should be carrying for its
// current rank and stacks, takes off exactly what it is carrying, and puts the right amount on.
// Idempotent - calling it twice costs nothing and a missed call is repaired by the next one,
// which matters because six different places in this engine write gridPos.
//
// The rank it reads is the DEPLOYED rank, so an in-fight reposition does not rewrite anyone's
// max health mid-fight; the swap is reconciled the next time the Outpost touches the line,
// which is where the bar is allowed to move.
function syncRankPerks(char) {
    if (!char || !char.isPlayer) return;
    const had = char.rankPerked || { hp: 0, spd: 0 };
    char.maxHp -= had.hp; char.speed -= had.spd;
    const hpStacks  = char.gridPos === 1 ? perkStacks(char, 'HARDENED') : 0;
    const spdStacks = char.gridPos > 1 ? perkStacks(char, 'SWIFT') : 0;
    const hp  = hpStacks ? Math.ceil(char.maxHp * (Math.pow(1 + PERK_MAXHP, hpStacks) - 1)) : 0;
    const spd = spdStacks * PERK_SPD;
    char.maxHp += hp; char.speed += spd;
    // A grant that grew hands over the difference, the way `c.hp += g` did; one that shrank
    // just clamps. Nobody is healed by being moved out of the front rank and back again.
    if (hp > had.hp) char.hp += hp - had.hp;
    char.hp = Math.max(0, Math.min(char.hp, char.maxHp));
    char.rankPerked = { hp, spd };
}
function syncAllRankPerks() { (playerRoster || []).forEach(syncRankPerks); }
...
function bankPerkStack(char, id) {
    ...
    // FORTIFIED asks nothing, so it is written straight onto the sheet the way it always was.
    if (id === 'FORTIFIED') { char.maxHp += PERK_HP_FLAT; char.hp += PERK_HP_FLAT; }
    syncRankPerks(char);
}
...
const PERK_POOL = [
    { id: 'VETERAN',   label: `VETERAN (+${PERK_DMG_FLAT} DMG, melee)`,
      desc: `+${PERK_DMG_FLAT} damage on every swing of a melee weapon. Nothing on a ranged one.` },
    { id: 'HONED',     label: `HONED (+${Math.round((PERK_DMG_MULT - 1) * 100)}% DMG, ranged)`,
      desc: `+${Math.round((PERK_DMG_MULT - 1) * 100)}% damage on everything fired at range. Nothing swung by hand.` },
    { id: 'HARDENED',  label: `HARDENED (+${Math.round(PERK_MAXHP * 100)}% MAX HP, front rank)`,
      desc: `+${Math.round(PERK_MAXHP * 100)}% max health while deployed in the front rank, and none of it anywhere else.` },
    { id: 'SWIFT',     label: `SWIFT (+${PERK_SPD} SPD, off the front)`,
      desc: `+${PERK_SPD} speed while deployed behind the front rank.` },
    { id: 'FORTIFIED', label: `FORTIFIED (+${PERK_HP_FLAT} HP)`,
      desc: `+${PERK_HP_FLAT} max health, wherever this operator stands and whatever it carries.` }
];
...
// K03: the elite gear drop was a bare 0.4 inside checkWinState, so 28-gear could only assert
// the rate it measured rather than the rate the engine asks for - and it drifted, because the
// measurement ran 60 fights without emptying the stash and rollGear thins as the pool is taken.
// Named here with the other rate dials so a suite can compare what it counts against what was
// asked for, the way ground and formations already are.
const ELITE_GEAR_CHANCE = 0.4;
...
// Seed to best score, which is what the daily line and the run-over screen read. The blob
// underneath holds a little more than that; seedBestRows is the whole of it.
function seedBests() {
    const rows = seedBestRows();
    const out = {};
    Object.keys(rows).forEach(k => { out[k] = rows[k].s; });
    return out;
}
// G09: a score and when it was last touched. The cap used to be enforced with `keys.sort()`,
// which throws away whatever sorts lowest rather than whatever is oldest - so a seed's fate
// depended on its name. The case that shows it: a favourite seed played first and then played
// again ten runs later was still the first thing evicted, because "AAA-" sorts below "zz-".
// The player's own table threw out the seed they kept coming back to.
//
// A blob written before this holds a bare number. It migrates to a row with no timestamp,
// which sorts oldest and is therefore evicted first - which is right, because it is oldest.
function seedBestRows() {
    const v = Store.getJSON(SEED_BEST_KEY);
    const raw = (v && v !== CORRUPT && typeof v === 'object') ? v : {};
    const out = {};
    Object.keys(raw).forEach(k => {
        const r = raw[k];
        out[k] = (r && typeof r === 'object')
            ? { s: Number(r.s) || 0, t: Number(r.t) || 0 }
            : { s: Number(r) || 0, t: 0 };
    });
    return out;
}
const SEED_BESTS_KEPT = 20;
function noteSeedBest(seed, score) {
    if (!seed) return null;
    const all = seedBestRows();
    const prev = all[seed] ? all[seed].s : 0;
    // Strictly above every stamp already in the table, so two writes in the same millisecond
    // still order - otherwise a tie falls back on the key, which is the defect again.
    const t = Math.max(Date.now(), ...Object.keys(all).map(k => all[k].t + 1), 0);
    // Touched, not just beaten. Coming back to a seed is what keeps it: the table is the twenty
    // the player is actually playing, and a run that fell short is still a run on that seed.
    // Only a better score moves the number.
    all[seed] = { s: Math.max(prev, score), t };
    const keys = Object.keys(all);
    if (keys.length > SEED_BESTS_KEPT) {
        keys.sort((a, b) => (all[a].t - all[b].t) || (a < b ? -1 : 1))
            .slice(0, keys.length - SEED_BESTS_KEPT)
...
const DOCTRINES = [
    ...
    { id: 'OLD_GUARD', name: 'OLD GUARD', bonus: 0.15,
      ...
      offerable: () => Object.keys(mastery || {}).filter(c => masteryRank(c) >= VETERAN_RANK).length >= OLD_GUARD_VETS,
...
const OLD_GUARD_VETS = 3;      // how many classes must reach it before the doctrine is offered
...
const CODEX = [
    ...
    { id: 'RESISTANCE', title: 'ARMOUR AND RESISTANCE', body: () => [
        'Everyone on the field carries three badges under their health - yours and theirs: P physical, B biological, E energy. A badge only appears where there is something to say.',
    ...
    { id: 'PROMOTIONS', title: 'FIELD PROMOTIONS', body: () => [
        ...
        'Training stacks, and four of the five pay nothing at all while their condition is off. Two read the weapon an operator carries - one rewards a blade, the other something with reach - and two read where it is deployed, one for the front rank and one for behind it. The fifth asks nothing and is the card to take when you have not decided what a body is for. Signatures and training both stay on an operator for good, so pick for the job that operator is going to keep doing.',
...
// Where the manual was opened from, so CLOSE can put it back. G10: that button went to the
// title, which was harmless while the only door to it was on the map - and a trap the moment
// the gear reached a fight, because closing the manual would have walked out of the run to
// look something up.
let codexFrom = null;
function renderCodex() {
    codexFrom = currentScreen();
...
const EVENT_POOL = [
    ...
    { title: "THE DEBT COLLECTOR", cast: 'VELA',
      ...
      choices: () => {
        ...
        // What the paper already on the table says, rather than what a new loan would cost.
        const owedNow = (pendingConsequences.find(c => c.kind === 'DEBT') || {}).amount || owed;
        const list = [];
        if (!owing) list.push(
          ...
          // Settling at the table, rather than waiting to be found. Vela's standing used to move
          // in exactly two places - the fuse coming due, and a shakedown that only exists once
          // you are already at BAD BLOOD - so from neutral there was nothing on this screen that
          // could raise her. Measured over 30 runs she reached a gate 0 times in 9 appearances
          // while every other face reached one in most of theirs. Paying early is the obvious
          // thing to offer a woman who lends for a living, and it is the move that opens VELA'S
          // LEDGER, the thread that wants standing 2 AND no outstanding paper.
          { label: `Settle up now (-${owedNow} Scrap)`, canAfford: () => scrap >= owedNow,
            execute: () => { scrap -= owedNow;
              pendingConsequences = pendingConsequences.filter(c => c.kind !== 'DEBT');
              noteCast('VELA', 2); playSFX('click');
              return `You put it on the hood before she asks. She counts it twice, writes it down as paid, and deals you back in.`; } },
          { label: "Ask for more (she refuses)", canAfford: () => false,
            execute: () => "She does not deal you in twice." });
        list.push(
          // Any one component, not the rarest one. This was Tech only, which is the material a
          // run is least likely to be holding, so the single at-the-table move she had was
          // itself usually greyed out.
          { label: "Sell her a favour instead (+80 Scrap, -1 component)",
            canAfford: () => MATERIAL_KINDS.some(k => materials[k] >= 1),
            execute: () => { const k = MATERIAL_KINDS.find(x => materials[x] >= 1);
              materials[k] -= 1; scrap += 80; noteCast('VELA', 1); playSFX('click');
              return "She takes the component, turns it over once, and pays you 80 Scrap for it."; } },
          // Something for a squad carrying neither scrap nor components: her regard, bought on
          // credit. In her idiom exactly - she is happy to be owed.
          { label: `Put your name to someone else's paper (owe 150 in ${DEBT_TERM} nodes)`,
            canAfford: () => !owesVela(),
            execute: () => { bookConsequence('DEBT', CONSEQUENCE_FUSE.DEBT, { amount: 150 });
              noteCast('VELA', 1); playSFX('click');
              return "She slides a second card across and taps the corner. 'You're good for it. That's the part that matters.'"; } },
...
// F15: what a settled contract looks like, wherever it settles. The board's own announcement
// was `if (combatActive) log(...)` - so CRAFT, the commonest contract there is, completed at
// the workbench and said nothing at all, and so did a bond formed at the Outpost, a face
// reaching TRUSTS and a sector crossed. A log line is also the wrong instrument off the field:
// there is no log on those screens to read it in.
let settleTimer = null;
function noteSettled(title, line) {
    const el = document.getElementById('settle-banner');
    if (!el) return;
    el.innerHTML = `<div class="settle-banner-name">${title}</div>`
                 + (line ? `<div class="settle-banner-desc">${line}</div>` : '');
    el.classList.remove('settle-banner-show');
    void el.offsetWidth;                       // restart it when two settle together
    el.classList.add('settle-banner-show');
    clearTimeout(settleTimer);
    // Cleared on a timer for the same reason the front banner is: where motion is off it is
    // shown rather than animated, and nothing else would take it down.
    settleTimer = setTimeout(() => { el.classList.remove('settle-banner-show'); el.innerHTML = ''; }, 2800);
}
// A settled contract is drawn once, struck through, and only then swapped out - so the
// rotation is a step of its own rather than part of the settlement. Every type on the board is
// excluded, including this slot's, so the board never hands back what it just paid, and it
// no-ops when nothing is claimed so both callers below can fire it unconditionally.
function rotateSettled() {
    if (!activeBounties || !activeBounties.some(b => b.claimed)) return false;
    // One slot at a time rather than one map over the board: two contracts can settle on the
    // same event - a KILL at 1 of 2 and another at 2 of 3 both finish on the same kill - and
    // the second roll has to see what the first was just handed, or the board shows a duplicate.
    for (let i = 0; i < activeBounties.length; i++) {
        if (activeBounties[i].claimed) activeBounties[i] = rollBounty(activeBounties.map(o => o.type));
    }
    return true;
}
...
function advanceBounties(type) {
    // Anything settled since the last call has had its render by now, so clear the board before
    // reading it. renderMap rotates too and normally gets there first; this is the belt for the
    // paths that never draw a map between two contract events - and for the simulator, which
    // draws nothing at all and would otherwise freeze every slot the moment it was paid.
    rotateSettled();
    if (activeBounties) activeBounties.forEach((b, idx) => {
        ...
        noteSettled('CONTRACT SETTLED', `${b.desc} \u00b7 +${b.reward} SCRAP`);
        // F15: marked rather than replaced. `claimed` was written once at creation, read in two
        // places and never set - so the flag and the struck-through style it drives were both
        // dead, and a finished contract was swapped out between renders with nothing to see.
        // The board draws it settled once and rotateSettled swaps it out after, so the fresh
        // contract is never handed back in the same breath the old one is paid.
        b.claimed = true;
    });
    ...
    if (standingBounty && standingBounty.type === 'S_' + type) {
        ...
        if (standingBounty.current >= standingBounty.target) {
            ...
            noteSettled('STANDING CONTRACT SETTLED', `${standingBounty.desc} \u00b7 +${standingBounty.reward} SCRAP`);
            firePrompt('STANDING');
...
// downTurns is only ever read while hp <= 0, so any heal that lifts them above zero is a save.
// F07: what that comment used to claim next - that the clock "is set fresh on every fall" - was
// the defect. goDown refuses to re-clock somebody who already carries a number, which is what
// stops a second hit on the floor from resetting the count; but the number was only ever
// cleared by the two paths that END a fall, loseOperator and the drag clear. An operator picked
// up mid-fight therefore walked away still carrying it, and their next fall - in that fight or
// any later one - hit that same guard and returned: never announced, no fresh clock, no
// Hazmat's Dead Man's Switch, and a stale 1 killed them on the first tick.
function isDown(e) { return !!e && e.isPlayer && !e.fallen && e.hp <= 0; }
// There is no choke point for healing - CAUTERIZE, STIM DART, two items, FIELD REVIVE and any
// squad heal that stops filtering on hp > 0 all reach the floor - so rather than ask a dozen
// callers to remember, the clock is swept where it can be SEEN to be stale. Above zero health
// there is no clock.
//
// Swept at the top of every turn, which is enough, and the argument is worth writing down
// because "enough" is the whole question. A pick-up happens during somebody's turn. For the
// number to still be stale at a fall, that fall would have to land in the SAME turn - and
// within one turn the only operator who can take damage is the one acting, through
// fireOverwatch, which aims at the actor. Somebody who has just been picked up is not the
// actor; they were on the floor, and the floor does not act. So every re-fall is at least one
// processTurn away from the heal that saved them. nextTurn's tick cannot reach a stale clock
// either: it is gated on isDown, and a stale clock belongs to somebody above zero.
function clearStaleClocks() {
    activeEntities.forEach(e => { if (e.isPlayer && e.hp > 0 && (e.downTurns || 0) > 0) e.downTurns = 0; });
}
...
// F10: HOLD ends the turn and grants nothing, and that is a measured decision rather than a
// shortcut. The phase asked for "a small, honest return, a guard tick or five momentum". Both
// were tried and both were wrong.
//
// Five momentum was wrong on inspection: a plain swing that does not kill grants NO momentum -
// the bar fills off kills, combos and blows taken - so a pass that paid it would be the
// cheapest overdrive charge in the game and worth pressing on purpose, which is the risk the
// phase itself names.
//
// A guard tick was wrong on measurement, which took three arms to establish. Runs that ended
// the road, 150 an expedition set:
//
...
//
// The first two are the same behaviour - the pre arm has no HOLD branch at all, so its pass is
// already a no-op - and they differ by 0.78 points, which is this row's noise floor at 150.
// Pooled as one arm they are 31/1350, 2.30%, against 0.78% with the guard: the guard is the
// only thing that separates, and removing it puts the row back on the baseline.
//
// The mechanism is NOT understood. Free armour on a squad lowering its win rate has no story
// behind it, and every other dial moved the player's way at the same time - score, nodes
// cleared, operators lost. What is established is narrow and it is enough to act on: those two
// lines cost runs, they are not needed for the thing the phase is for, and a pass that quietly
// costs the run is worse than a pass that pays nothing. Left here as a finding rather than a
// fix, because a fix would need the mechanism first.
function executeSelfAction(type) {
    ...
    if (type === 'HOLD') {
        log(`> ${actEnt.name} holds the line.`, "log-status");
...
const MATERIAL_KINDS = Object.keys(MATERIAL_ICON);
...
function renderChronicle() {
    ...
    // The dossiers, a line per class. Mastery is a lifetime, cross-run progression that gates
    // three things - a class quirk at rank II, a fourth ability at rank III, and two doctrines
    // that one of them counts veterans for - and its numbers were only ever printed on cards from
    // playerRoster. buildNewRun fills that from ROSTER_TEMPLATE, which is seven entries, so the
    // Trench Fiend, the Hazmat and the Harpooner could climb for a whole career with no figure
    // ever shown: with every class at rank III lifetime XP the Outpost renders seven lines and
    // those three are not among them. The codex's own DOSSIERS entry lists thresholds and unlock
    // names and reads neither masteryXp nor masteryRank, so it says what the system is and
    // nothing about where this player stands in it.
    //
    // Surfacing only. Nothing here changes what mastery does or when it does it.
    const MARKS = ['\u00B7', '\u2605', '\u2605\u2605', '\u2605\u2605\u2605'];
    const vets = Object.keys(MASTERY_TITLES).filter(c => masteryRank(c) >= VETERAN_RANK);
    document.getElementById('chronicle-dossiers').innerHTML =
        `<div class="dossier-head">DOSSIERS \u00B7 ${Object.keys(MASTERY_TITLES).filter(c => masteryRank(c) >= 1).length} of ${Object.keys(MASTERY_TITLES).length} RANKED</div>` +
        Object.keys(MASTERY_TITLES).map(cls => {
            const xp = masteryXp(cls), r = masteryRank(cls);
            ...
            return `<div class="dossier-row dossier-r${r}">
                ...
                <span class="dossier-xp">${xp.toLocaleString()}${next ? ` / ${next.toLocaleString()}` : ''}</span>
                <span class="dossier-next">${unlock}</span>
            </div>`;
        }).join('') +
...
function heirloomFrom(relics) {
    ...
    // Rares first, then anything clean, and a curse only when a curse is all there is. G07: the
    // fallback used to be relics[0], so a squad holding a curse it picked up before a common
    // banked the curse - the Vault is a building the player bought, and it was arming the next
    // run with a penalty. A curse can still be banked when it is the only thing in the bag, and
    // it leaves the moment the next run finds anything cleaner, because the Vault re-reads what
    // was held at every run's end.
    return relics.find(r => r.tier === 'RARE')
        || relics.find(r => r.tier !== 'CURSED')
        || relics[0];
}
...
// What holding the field pays. Extracted from checkWinState, where it was computed inline on the
// way to writing a LOOT button - which meant the only way to find out what a fight was worth was
// to press that button, and the headless harness, which cannot, kept a private formula instead.
// Its copy paid a flat 20 base where this rolls 0-29, and knew nothing about the front's ledger,
// the vulture's cut, the magnet or the collector's debt.
function fightPayout() {
    const scrapMult = isCurrentNodeElite ? 2 : 1;
...
// Everything a cleared node changes, with no screen in it: the withdrawal is disarmed, a rank
// left empty is closed from the bench, RATIONING takes its cut, the pay lands, the tier ticks
// over and the save is written. collectLoot is this plus the screen chain that follows; the
// harness is this without. Split so neither can drift from the other - the reason RATIONING and
// closeRanks had never once run on a won fight in simulation is that both lived behind a button.
function bankNode(raw, abandoned) {
    disarmWithdraw();
    closeRanks();
...
// G07: "marked unmistakably, never forced" is rollRelicOffer's own rule for a cursed card, and
// the screen had no way to keep it. The table is built from what the squad does NOT already
// own, so a career deep enough to hold thirteen of the fourteen clean relics is dealt one card
// - and the curse roll replaces the first one, which is that card. Measured: offer size 1,
// tier CURSED, one button on the screen, no exit at all. A player who wants none of what is on
// the table can leave with nothing; that is what it costs, and there is no second price.
function declineRelic() {
    const had = (pendingRelicOffer || []).length;
    log(had ? `> You look at what the commander was keeping, and leave it where it is.`
            : `> The stash is bare.`, 'log-status');
    leaveOffer();
}
// One exit for both answers, so taking a card and leaving it cannot drift apart on what
// happens next - the promotion behind the offer is the thing that used to get lost.
function leaveOffer() {
...
// G10: the gear was on three screens - map, outpost, citadel - and it is the only door to the
// settings panel and the only door to the FIELD MANUAL behind it. So a player could not turn
// the sound down mid-fight, could not slow the combat speed, and could not look up what a
// resistance badge meant at the one moment the badge was in front of them. Nor from the title,
// where there was no way in at all.
//
// Everywhere now, except the two screens that ARE the settings: the panel itself, and the
// manual the panel opens on top of. Both measured before it moved - nothing on any screen sits
// under the gear's 44x44 corner, and the panel already overlaid a fight without touching it
...
function currentScreen() {
    const el = [...document.querySelectorAll('#engine > div[id^="screen-"]')]
        .find(e => e.id !== 'screen-settings' && getComputedStyle(e).display !== 'none');
    return el ? el.id : null;
}
// F14: the screen changed under the caret and nothing moved with it, so a keyboard was left
// tabbing from wherever the last screen had got to - usually the top of the document. The
// screen itself takes focus rather than its first button: announcing "map" and letting the
// player tab in reads better than dropping them onto one control out of twenty, and it does
// not steal the press from a button that was just used to get here.
function focusScreen(screenId) {
    const el = document.getElementById(screenId);
    if (!el || typeof el.focus !== 'function') return;
    el.setAttribute('tabindex', '-1');
    if (!el.hasAttribute('aria-label')) el.setAttribute('aria-label', screenId.replace('screen-', '').replace(/-/g, ' '));
...
function closeCodex() {
    // Only closes the manual if the manual is what is up. Without this a stray call - a stale
    // click, a rebound handler - would fire the "go back" with whatever the last visit left
    // behind and jump the player somewhere they never asked for.
    if (currentScreen() !== 'screen-codex') return;
    const back = codexFrom;
    codexFrom = null;
    if (back && back !== 'screen-codex' && back !== 'screen-title' && document.getElementById(back)) {
        switchScreen(back);
        return;
    }
    renderTitleScreen();
...
function renderMap() {
    ...
    // F15: drawn settled above, rotated out here - so the row is struck through on the render
    // that follows the settlement and replaced on the next one.
    if (rotateSettled()) saveGameState();
...
// The crossing itself, with no screen in it. advanceSector is this plus the consequence chain,
// which paints and then falls through to afterNode when nothing is due - so a headless caller
...
function operatorCardHtml(char) {
        ...
        // K04: what this body shrugs off, on the screen where the trinket that buys one is put
...
// A level point is what an operator's identity is bought with, and until now the Outpost's
// spend path did not know signatures existed: assignPerk resolved against PERK_POOL only, so a
// signature id handed to it fell out of `if (!perk) return` without spending the point or
// granting anything. Three separate sources bank points that could only ever become +5 DMG -
// the BANK button, the Barracks head start (four levels at its cap, four points, and a pitch
// that says "grants early Perk point"), and the Oracle event.
//
// It resolves both now. Two things keep the promotion screen the better place to be standing:
// the offer is free where this costs scrap, and the offer is where a fork is presented as the
// either/or it is. Buying a half here closes the other half exactly as taking it there does.
const SIG_BUY_BASE = 60;
function sigBuyCost() { return Math.round(SIG_BUY_BASE * sectorRewardMult()); }
...
// K02: for a long time the recruit card was the ONLY surface in the game that printed a
// player-side resistance figure - the roster card did not, and resistBadges returned nothing for
// an operator - and it printed `+25% bio` against arithmetic that subtracts flat. Quoted in the
// words the gear that sells one already uses ("+10 bio resist"), with a negative reading named
// as the weakness it is rather than as a resistance with a minus in front of it.
//
// K04: and the roster card says it too now, which is where the trinket that buys one is actually
// equipped. One function rather than two copies of the wording: a second copy is the F03 defect
// wearing a different hat, and these two would drift the first time either was reworded.
function resistLine(res) {
    if (!res) return '';
...
function recruitCardHtml(tpl) {
    ...
    const res = resistLine(tpl.resistances);
...
function intentLegendHtml() {
    const live = [...new Set(activeEntities.filter(e => !e.isPlayer && e.hp > 0 && e.intent)
        .map(e => e.intent.type))].filter(t => INTENT_WORDS[t]);
    if (!live.length) return '';
    return `<div class="intent-legend" aria-label="What the icons over each enemy mean">`
        + live.map(t => `<span class="intent-legend-item">${INTENT_ICONS[t] || ''} <b>${INTENT_WORDS[t]}</b></span>`).join('')
        + `</div>`;
}
...
function log(msg, styleClass = "log-dmg", hitId = null) {
    ...
    // F14: a plain div with a click handler is a control only for a mouse. The arithmetic
    // behind a blow is the one surface that answers "why did that number happen", and it was
    // unreachable from a keyboard.
    if (hitId !== null) {
        el.classList.add('log-explainable'); el.dataset.action = 'explain'; el.dataset.hit = String(hitId);
        el.tabIndex = 0; el.setAttribute('role', 'button');
        el.setAttribute('aria-label', `Explain: ${msg}`);
    }
...
function renderField() {
    ...
    activeEntities.forEach(ent => {
        ...
        if (pendingAction) {
            // F07: `ent.isPlayer` alone offered the MEDIC's FIELD REVIVE every body on the
            // field, the lost included. loseOperator takes them off the roster but deliberately
            // leaves the corpse here, because the LOST tag below is drawn on it - so the fix is
            // to stop aiming at them, not to remove them.
            if (pendingAction === 'OVERDRIVE' && turnQueue[activeIndex].classType === 'MEDIC' && ent.isPlayer && !ent.fallen) {
...
function renderCommandDeck() {
    ...
    if (momentum >= overdriveAt() && !hasScar(aE, 'GUN_SHY')) {
        ...
        if (!odChoices[aE.classType] && pair.length === 2) {
            ...
            // F14: what each one DOES was in a title tooltip, which does not exist on a touch
            // screen - so the one-time choice that locks the class for the rest of the run was
            // being made off two names. The line is in the deck now, where the thumb is.
            deckHtml += `<div class="od-choice-note">One of these, and it is the ${aE.classType.replace(/_/g, ' ')}'s for the rest of the expedition.</div>`;
            pair.forEach(o => {
                deckHtml += `<button class="title-btn btn-overdrive" data-action="queue" data-move="OVERDRIVE" data-variant="${o.id}" title="${o.desc}">OVERDRIVE: ${o.name}</button>`
                          + `<div class="od-choice-note">${o.desc}</div>`;
            });
        } else {
...
function resolveAction(targetId) {
    ...
    if (pendingAction === 'OVERDRIVE') {
        ...
        if (variant.id === 'EARTHSHAKER') {
            ...
            // The belt to the render's brace. "Revive" means off the floor, not out of the
            // ground: somebody lost for good is off the roster, and raising them here put a
    ...
    if (pendingAction === 'CAUTERIZE') {
        ...
        // M04: hoisted, because VETERAN and HONED both key on which kind of weapon this
        // operator is actually holding and baseDmg is figured before the reach layer runs.
        const effReach = moveReachFor(pendingAction, actEnt);
        let baseDmg = actEnt.dmgBase + perkDmgFlat(actEnt, effReach) + tuneUpBonus + Math.floor(Math.random() * 6); 
...
function awardXp(char, amount) {
    ...
    while (char.xp >= char.xpToNext) {
        ...
        // D11: four signatures exist per class, and every level past the fourth used to pop the
        // same interrupt anyway - three cards drawn from the same five flat stat bonuses, with
        // nothing left to actually decide between drawing them now over any other time. Measured
        // over a run's ~42 promotions, 16 were exactly this: a screen with no signature left to
...
function checkWinState() {
    ...
    if (!pA) { document.getElementById('command-deck').innerHTML = `<button data-action="squad-down">SQUAD DOWN</button>`; combatActive = false; stopAmbience(); } 
        ...
        if (isCurrentNodeElite) {
            ...
            if (hasRelic('VULTURE_ROYALTY') || Math.random() < ELITE_GEAR_CHANCE) {
